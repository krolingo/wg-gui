...
# Full copy of the original wg-quick.bash goes below...
# (Download it from https://git.zx2c4.com/WireGuard/tree/src/tools/wg-quick/bash/wg-quick.bash)
# Example top part:
set -e
shopt -s extglob
...
CONFIG_SEARCH_PATHS=( /usr/local/etc/wireguard ~/scripts/wireguard_client/profiles )
...
cmd_up() {
	...
	set_config
    # Save the loaded profile to wg0.conf for consistency with the GUI
    cmd cp "$CONFIG_FILE" "/usr/local/etc/wireguard/wg0.conf"
...
#
# Override: if passed a full path to a config file, treat it literally
if [[ $# -eq 2 && ( $1 == up || $1 == down || $1 == save || $1 == strip ) && "$2" == /*.conf ]]; then
  auto_su
  parse_options "$2"
  case "$1" in
    up) detect_launchd; cmd_up ;;
    down) cmd_down ;;
    save) cmd_save ;;
    strip) cmd_strip ;;
  esac
